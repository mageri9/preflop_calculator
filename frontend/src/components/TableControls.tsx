...
export interface TableControlsProps {
  ...
  onNextHand: () => void;
  onResetSession: () => void; // <--- Вернули проп
  onChangeTableSize: (newSize: number) => void;
  onUpdateSession: (payload: Partial<TableSession> & { stack_bb?: number }) => void;
  isLoading?: boolean;
}
...
export function TableControls({
  ...
  const commitBBValue = (valueStr: string) => {
    ...
    if (!isNaN(parsed) && parsed > 0) {
      onUpdateSession({ stack_bb: parsed });
    } else {
      setBbInput(String(session.stack_bb));
    }
  };
  ...
  const changeStackBB = (deltaBB: number) => {
    const newBB = Math.max(1, Math.round(session.stack_bb + deltaBB));
    setBbInput(String(newBB));
    onUpdateSession({ stack_bb: newBB });
  };

  const isMaxLevel = session.blind_level >= MAX_BLIND_LEVEL;

  return (
    <section className="mx-auto w-full max-w-[360px] space-y-1.5">
      {/* Кнопка сдачи раздачи */}
      <button
        type="button"
        disabled={isLoading}
        onClick={onNextHand}
      ...
      <div className="flex gap-1.5">
        ...
        <button
          ...
          className="rounded-lg border border-slate-800 bg-slate-900 px-2.5 py-1.5 text-[10px] font-bold text-slate-400 hover:text-white"
          title="Сбросить турнир к начальному стеку"
        >
          Сброс
        </button>
      </div>
        ...
        <div className="grid grid-cols-2 gap-1.5 rounded-lg border border-slate-800 bg-slate-900 p-2 text-[10px]">
          ...
          <label className="space-y-1 text-slate-500">
            ...
            <select
              ...
              disabled={isLoading}
              onChange={(event) =>
                onUpdateSession({ icm_stage: event.target.value as TableSession['icm_stage'] })
              }
              className="block w-full rounded bg-slate-800 p-1.5 text-slate-200"
            >
          ...
          <label className="space-y-1 text-slate-500">
            ...
            <select
              ...
              disabled={isLoading}
              onChange={(event) =>
                onUpdateSession({ opponent_style: event.target.value as TableSession['opponent_style'] })
              }
              className="block w-full rounded bg-slate-800 p-1.5 text-slate-200"
            >
